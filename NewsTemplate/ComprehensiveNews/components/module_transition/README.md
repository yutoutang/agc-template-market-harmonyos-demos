...
2. 引入组件。
   ...
   ```
   import { LongTakeTransitionDelegate,ImageLongTakeDelegate } from 'module_transition';
   ```
...
LongTakeTransitionDelegate(option: [LongTakeTransitionDelegateOptions](#LongTakeTransitionDelegateOptions对象说明))
...
import { LongTakeAnimationProperties, LongTakeTransitionDelegate, LongTakeTransitionParam } from 'module_transition';
...
```typescript
// 在项目src/main/ets/entryability/EntryAbility.ets的onWindowStageCreate中添加init逻辑
onWindowStageCreate(windowStage: window.WindowStage): void {
