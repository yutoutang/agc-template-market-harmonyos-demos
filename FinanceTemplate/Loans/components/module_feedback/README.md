...
1. 安装组件。
   ...
   b. 在项目根目录build-profile.json5添加module_feedback、module_image_preview模块。

   ```
   // 项目根目录下build-profile.json5填写module_feedback、module_image_preview路径。其中XXX为组件存放的目录名
   "modules": [
     {
     ...
     {
       "name": "module_image_preview",
       "srcPath": "./XXX/module_image_preview"
     }
   ]
   ```
