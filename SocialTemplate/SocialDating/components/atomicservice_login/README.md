...
2. 引入元服务关联账号组件句柄。
   ...
   ```
   import { AccountUtils, AtomicserviceLogin, ApiController, CancelAuthState, PersonalInfo } from "atomicservice_login";
   ```
...
| 参数名     | 类型                                                          | 是否必填 | 说明             |
|:--------|:------------------------------------------------------------|:-----|:---------------|
| options | [AtomicserviceLoginOptions](#AtomicserviceLoginOptions对象说明) | 否    | 元服务关联账号组件的参数。 |

#### AtomicserviceLoginOptions对象说明

| 参数名        | 类型                                      | 是否必填 | 说明                |
|:-----------|:----------------------------------------|:-----|:------------------|
| PersonalInfo   | [userInfoOptions](#userInfoOptions对象说明) | 是    | 用户信息              |
...
| 实例名          | 接口名                                                                                                              | 说明       |
|:-------------|:-----------------------------------------------------------------------------------------------------------------|:---------|
| AccountUtils | silentLogin(callback: (data: authentication.LoginWithHuaweiIDCredential \| undefined) => void): Promise<Boolean> | 静默关联账号以及回调 |
...
关联手机号码的回调，返回用户的临时关联账号凭证（Authorization Code），可通过临时关联账号凭证获取真实手机号，临时关联账号凭证时效5分钟，具体操作可参考“[服务端开发](https://developer.huawei.com/consumer/cn/doc/atomic-guides/account-guide-atomic-get-phonenumber#section380015370555)”章节。
...
```ts
import { AtomicserviceLogin, ApiController, PersonalInfo } from "atomicservice_login";

@Entry
@ComponentV2
struct AtomicserviceLoginPage {
  @Local userInfo: PersonalInfo = new PersonalInfo()
