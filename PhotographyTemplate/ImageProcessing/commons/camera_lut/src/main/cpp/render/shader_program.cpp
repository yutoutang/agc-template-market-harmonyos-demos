...
ShaderProgram::~ShaderProgram() noexcept {
    if (Valid()) {
        glDeleteProgram(id_);
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "RenderThread", "ShaderProgram 析构完成.");
    }
}
...
void ShaderProgram::CheckCompileErrors(GLuint shader, const std::string &shaderType) {
    ...
    if (shaderType != "PROGRAM") {
        ...
        if (!success) {
            ...
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ShaderProgram",
                         "ERROR::PROGRAM_LINKING_ERROR of type: %{public}s, infoLog is: %{public}s", shaderType.c_str(),
                         infoLog);
        }
    }
}
